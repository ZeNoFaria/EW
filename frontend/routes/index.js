  ...
  try {
    ...
    try {
      ...
      if (entriesResponse.data.pagination) {
        ...
        rawEntries =
          entriesResponse.data.entries || entriesResponse.data.data || [];
      }
    } catch (timelineError) {
    ...
    try {
      ...
      const uniqueTags = [
        ...new Set(
          allTags.map((tag) => (typeof tag === "object" ? tag.name : tag))
        ),
      ];
    ...
    if (paginationData) {
      finalPaginationData = {
        prevUrl: paginationData.hasPrevPage
          ? `/timeline?page=${paginationData.prevPage}${
              tag !== "All" ? `&tag=${tag}` : ""
            }`
          : null,
        nextUrl: paginationData.hasNextPage
          ? `/timeline?page=${paginationData.nextPage}${
              tag !== "All" ? `&tag=${tag}` : ""
            }`
          : null,
        pages: [],
    ...
    res.render("timeline", {
      ...
      shareButtonText: "Share",
      noEntriesMessage:
        "Unable to load entries. Please check your API connection.",
      pagination: null,
      scripts: ["javascripts/tag-filter.js"],
    });
  }
...
router.get("/entry/:id", async (req, res) => {
  try {
    ...
    console.error(
      `Error fetching entry ${req.params.id} from API:`,
      err.message
    );
    console.error("Full error:", err.response?.data || err);
...
router.get("/categories", async (req, res) => {
  try {
    ...
    const lastItemDate = categories.reduce((latest, cat) => {
      if (
        cat.lastUpdate &&
        (!latest || new Date(cat.lastUpdate) > new Date(latest))
      ) {
    ...
    try {
      ...
      try {
        ...
        entries.forEach((entry) => {
          const categoryName =
            entry.category?.name || entry.category || "Uncategorized";
          ...
          if (
            !category.lastUpdate ||
            entryDate > new Date(category.lastUpdate)
          ) {
        ...
        console.error(
          "Could not fetch entries for categories:",
      ...
      try {
        ...
        tags = Array.from(tagMap.entries())
          .map(([name, count]) => ({
            ...
            size: Math.min(
              5,
              Math.max(1, Math.ceil(count / Math.max(1, tagMap.size / 5)))
            ),
          }))
          .sort((a, b) => b.count - a.count);
      } catch (entriesError) {
        console.error(
          "Could not extract tags from entries:",
          entriesError.message
        );
      }
    }
    ...
    const lastItemDate =
      categories.length > 0
        ? categories.reduce(
            (latest, cat) =>
              !latest ||
              (cat.lastUpdate && new Date(cat.lastUpdate) > new Date(latest))
                ? cat.lastUpdate
                : latest,
            null
          )
...
router.get("/category/:slug", async (req, res) => {
  try {
    ...
    const categoryName =
      entries.length > 0 && entries[0].category?.name
        ? entries[0].category.name
        : slug.replace(/-/g, " ").replace(/\b\w/g, (l) => l.toUpperCase());

    res.render("timeline", {
      // Reuse timeline template
      ...siteConfig,
